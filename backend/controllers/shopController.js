...
// @desc    Delete shop and all its products/reviews
// @route   DELETE /api/shops/:id
// @access  Retailer (owner only)
exports.deleteShop = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: 'Shop and all associated products & reviews deleted successfully'
    });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
...
// @desc    Add a photo to shop
// @route   POST /api/shops/photos
// @access  Retailer (owner only)
exports.addShopPhoto = async (req, res) => {
  try {
    ...
    shop.photos.push({ url, caption: caption || '' });
    await shop.save();
...
// ==========================================
// ✅ NEW: DELETE SHOP PHOTO
// ==========================================

// @desc    Delete a photo from shop
// @route   DELETE /api/shops/photos/:photoId
// @access  Retailer (owner only)
exports.deleteShopPhoto = async (req, res) => {
  try {
    ...
    const photoIndex = shop.photos.findIndex(
    ...
    shop.photos.splice(photoIndex, 1);
    await shop.save();
