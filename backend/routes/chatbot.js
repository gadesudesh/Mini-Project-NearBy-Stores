...
// ✅ Fixed: using GEMINI_API_KEY to match your .env file
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

// ✅ Fixed: gemini-1.0-pro is shut down, updated to gemini-2.0-flash
...
User: ${message}`;
    ...
    if (error.message && error.message.includes('API key not valid')) {
      errorMessage = 'The Google API key is invalid. Please get a new API key from https://aistudio.google.com/app/apikey';
    } else if (error.message && error.message.includes('is not found for API version')) {
      errorMessage = 'The API key is not enabled for Gemini API. Please get a key from https://aistudio.google.com/app/apikey';
    } else if (error.message && error.message.includes('quota')) {
      errorMessage = 'Sorry, the AI service is currently unavailable due to quota limits. Please check back later.';
    }
